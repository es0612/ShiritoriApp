...
struct ShiritoriIntegrationTests {
    ...
    func testLongVowelMarkConnection() async throws {
        ...
        #expect(ruleEngine.canWordFollow(previousWord: "すーぷ", nextWord: "ぷりん")) // 「すうぷ」→「ぷりん」
    ...
    func testHiraganaConversionWithNormalization() async throws {
        ...
        // カタカナ入力の正規化
        #expect(converter.convertToHiraganaForShiritori("ルビー") == "るびい")
        #expect(converter.convertToHiraganaForShiritori("スープ") == "すうぷ")
        
        // 混在文字列の処理
        #expect(converter.convertToHiraganaForShiritori("ジュース") == "じゆうす")
        
        // 既にひらがなの場合の正規化
        #expect(converter.convertToHiraganaForShiritori("すーぷ") == "すうぷ")
        #expect(converter.convertToHiraganaForShiritori("せんせー") == "せんせえ")
    }
    ...
    func testEdgeCaseIntegration() async throws {
        ...
        // 空文字列の処理
        #expect(converter.convertToHiraganaForShiritori("") == "")
        #expect(!ruleEngine.canWordFollow(previousWord: "", nextWord: "あ"))
        
        // 単一文字の処理
        #expect(converter.convertToHiraganaForShiritori("あ") == "あ")
        #expect(!ruleEngine.canWordFollow(previousWord: "ねこ", nextWord: "あ")) // 「こ」→「あ」は接続しない
        
        // 複雑な長音符・拗音組み合わせ
        let complexWord = "しゃーぷ" // 「しやあぷ」になる
    ...
    func testVoiceRecognitionSimulation() async throws {
        ...
        for (input, expected) in zip(voiceInputs, expectedOutputs) {
            let result = converter.convertToHiraganaForShiritori(input)
            #expect(result == expected, "音声入力'\(input)'の変換結果が期待値'\(expected)'と一致しません。実際: '\(result)'")
        }
    }
